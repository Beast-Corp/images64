...
Drop a high-resolution 1024×1024 PNG image named `icon.png` into the `/src-tauri/icons/` directory. This is the canonical source for all platform-specific icons. For release builds, the required OS icons (`.icns`, `.ico`, and various PNG sizes) are automatically generated by running:
```bash
